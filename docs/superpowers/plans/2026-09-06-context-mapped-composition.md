...
- [x] Write a public parent/child journey with present-nil and nested mapped inputs, an unmapped secret, child output overwrite, and a second missing output. Prove deep isolation and all-or-nothing outputs; missing inputs create no child root. Exercise success/partial success/failure/cancellation exact outcome propagation, exact missing mapping error shape/message, and opaque nested input path remapping.
- [x] Run focused test and observe absent handler behavior.
- [x] Implement a marked default placeholder in the registry, replaced only in the engine's cloned registry by a run-scoped handler. Explicit custom `subpipeline` entries win. Resolve the child through selected parent-plan reference/node ID. Deep-copy only mapped inputs into new Context; create UUID child roots beneath parent node `children/`; call injected execute-subpipeline callback with selected plan/context/root/registry/cancellation/events/budget. Gather every output before returning one context_updates map. Preserve failure/cancel category/retryability/reason/notes without outputs. Low-level calls without workflow/callback return nonretryable subpipeline_configuration_error.
- [x] Run composition, handlers, lifecycle, context isolation, status, and engine files; verify parent registry template stays unchanged and sibling child runtime atoms differ.
- [x] Commit `feat(composition): execute isolated mapped child workflows`.

Task 5 evidence: `ea08d98`, both review gates approved. Initial mapping RED 2 tests / 13 assertions / 8 failures; final mapped suite 16/114/0. Affected six-suite native run passes 113/871/0. Root independently verified the final full suite 463/3041/0, local let-go AOT build, compiled two- and three-level output mapping with distinct selected-plan checkpoints, and completed-parent resume after child-source drift without new child attempts. Pre-review regressions cover child failure metadata, node-timeout cancellation inheritance, raw/escaping/dangling status paths, and preexisting UUID child roots. Dangling-link RED: four failures; UUID collision RED: two failures. Task 6's deeper interruption/cancellation/isolation proofs remain open.
...
- [x] Commit, fast-forward main while preserving unrelated changes/worktrees, run full suite/AOT on main, push public main and verify remote commit. Next roadmap work remains ITER-0006.

Task 7 integration evidence: post-merge main full suite 473/3370/0, local let-go AOT build exit 0, fresh compiled composition example exit 0 with persisted `child.result = "hello-from-child"`. Public main was verified at `7d097b4` and the iteration branch at `ac7b241` using `git ls-remote`. Private notes remain untracked and all worktrees are preserved. The scoped delivery retains ATTR-READ-01 and ATTR-COMPOSE-02 as full-goal residuals.
