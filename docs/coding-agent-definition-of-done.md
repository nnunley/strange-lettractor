...
| 9.12 Parity matrix | `agent_parity_matrix_wire_test.lg`: every row for openai, anthropic and gemini with a scripted model through the real adapters; live rows via `make live-parity` (`test/live/parity_matrix.lg`): 8/8 on `llamacpp/qwen3.8-27b`, on the OpenAI Responses protocol (`or-responses/openai/gpt-4.1-mini`) and on the Anthropic Messages protocol (`or-messages/anthropic/claude-haiku-4.5`); the Gemini native cell awaits a key |
